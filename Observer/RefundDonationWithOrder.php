...
class RefundDonationWithOrder implements ObserverInterface
{
        ...
        try {
            ...
            if ($donationAmount) {
                ...
                if ($donationStatus != DonationManagementInterface::STATUS_SENT) {
                    $donation->setStatus(DonationManagementInterface::STATUS_REFUNDED);
                    $this->donationRepository->save($donation);
                }
                // make Delete request to Donmo API only if donation is already in SENT status,
                // then refund it locally if the deletion was successful
                else {
            ...
            $this->logger->error("Donmo RefundDonationWithOrder Observer error:\n" . $e);
        }
    }
}
